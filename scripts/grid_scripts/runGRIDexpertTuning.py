...
if clusterManagerConf() is ClusterManager.Panda:
  ...
  tuningExpertParser.delete_arguments( 'outputFileBase', 'data', 'crossFile', 'confFileList'
                                  ...
                                  , 'ppFile', 'outputDir' )
  tuningExpertParser.suppress_arguments(compress = 'False')
  ...
  # Suppress/delete the following options in the main-job parser:
  tuningExpertParser.delete_arguments( 'outputFileBase', 'confFileList'
...
parser = ArgumentParser(description = 'Tune discriminators using input data on the GRID',
    parents = [tuningExpertParser, parentParser, clusterParser, loggerParser],
...
if clusterManagerConf() is ClusterManager.Panda: 
  ...
  setrootcore_opts = ''
  tuningJob = os.path.join(rootcorebin, 'user_scripts/TuningTools/standalone/runExpertTuning.py')
  dataCaloStr, dataTrackStr, netCaloStr, netTrackStr, configStr, ppStr, crossFileStr, refStr, subsetStr = \
      args.data_calo, args.data_track, args.network_calo, args.network_track, '{CONFIG_FILES}', args.ppFile, args.crossFile, args.refFile, args.clusterFile
...
for etBin, etaBin in progressbar( product( args.et_bins(), 
  ...
  args.setExec("""{setrootcore} {setrootcore_opts}
                  {tuningJob} 
                    ...
                    --network-calo {NET_CALO}
                    --network-track {NET_TRACK}
                    --confFileList {CONFIG}
                    --ppFile {PP}
                    --crossFile {CROSS}
               ...
               """.format( setrootcore      = setrootcore,
                           ...
                           tuningJob        = tuningJob,
                           DATA_CALO        = dataCaloStr,
                           DATA_TRACK       = dataTrackStr,
                           NET_CALO         = netCaloStr,
                           NET_TRACK        = netTrackStr,
